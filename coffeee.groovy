/**
 *  Coffee Maker Handler
 *
 *  Copyright 2018 Example User
 *
 ...
 */
metadata {
	definition (name: "Coffee Maker Handler", namespace: "example user.coffee", author: "Example User") {
		capability "Switch"
...
def toggle() {
	def cmds = []
	cmds << http_command("/coffee/toggle/activate")
    log.debug cmds
	sendEvent(name: "toggle", value: "pushed", data: [buttonNumber: "1"], descriptionText: "$device.displayName Coffee Maker button was pushed", isStateChange: true)
    return cmds
}
